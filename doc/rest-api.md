...
## Purpose
Exposes project, ticket, comment, member, label, column, automation, and durable local-media job data over HTTP so that AI agents (and the Blazor UI) can operate the board programmatically.
...
- `GigaClaw.Web/Api/Endpoints.cs` — `MapApiEndpoints` entry point; route definitions are split across per-domain `partial class Endpoints` files in the same folder:
  - `Endpoints.Projects.cs`, `Endpoints.Tickets.cs`, `Endpoints.Columns.cs`, `Endpoints.Labels.cs`, `Endpoints.Members.cs`, `Endpoints.Automations.cs`, `Endpoints.Runs.cs`, `Endpoints.Chat.cs`, `Endpoints.Dashboard.cs`, `Endpoints.Skills.cs`, `Endpoints.Images.cs`, `Endpoints.Browse.cs`, `Endpoints.Media.cs`.
...
- `/api/projects/{slug}/media/jobs...` — submit idempotent local-media jobs, inspect status, cancel queued/running work, and record approval or rejection. See [Local media creation](./local-media.md).
