...
| C7 | **Landed** — branch `worktree-c5-c7-c8` | GitHub surface in three commits (`394cdb3`, `19209c2`, `59e40fe`), optional and off unless a project configures it — local-first stays the default. **Issue import:** labeled issues become tickets through a poll-based sync with an `github_issue_links` mapping table (inline migration) in the per-project DB, so a second sync or a restart creates nothing twice; closing the ticket comments on/closes the issue only behind a config flag. **PR owner feedback:** a new `gitHubPrComment` trigger re-dispatches the assignee with the owner's comment as steering input through the same dispatch-injection rail C3's FIX findings use — no second mechanism; automations can narrow the owner-login list but never widen it. **CI status:** `gitHubCheckStatus` joins the gitCommit trigger family, resolving commits the same way `gitCommit` does and keeping seen-state in `SessionRegistry` (the family's actual store — `TriggerStateStore` only holds `NextRunAt`), so dedupe survives a restart and a rerun that flips the conclusion is a new event. PAT lives in settings only — tests prove it cannot reach ticket content, receipts, URLs, or transport errors (plus the complement: it *does* reach the Authorization header). Every outbound call passes the P3 `OutboundApprovalGate`: blocked verdicts stop the sync before anything is sent, refusals leave `outbound-denial/v1` receipts, and approving `api.github.com` does not open a lookalike host. Five REST routes in `Endpoints.GitHub.cs`; Blazor settings UI deliberately not built (REST-only config, noted in the doc); triggers registered in the editor/palette (en/fr/es) because unregistered types throw there. `AutomationEngine`/`ProjectRuntimeManager` take *optional* GitHub services — a host that never registers them builds every other trigger unchanged, and a GitHub trigger degrades to a warned `NullTrigger` rather than failing the project's config load. [`doc/github-surface.md`](../github-surface.md). 56 new tests (1131→1187 on its own branch; 1230 after merging with C5) |
