...
- **7b Passes 2+3 LANDED before session close** (merged into `t6-packs`; union bar: replay **39/39**, judge **39/39** all `baseline=match`, eval **44/0**, core **1339/0**, strict 0). All of D3–D10 executed per the sheet. Corpus census now **39 entries — 38 SHIP / 0 FIX / 1 BLOCK**, with exactly one non-empty `vetoItems` (`render-without-sign-off` on the new D8 fixture `media-render-before-sign-off`). **One principled deviation the owner should ratify: D7 flipped FIX→SHIP, not FIX→BLOCK** — a scenario that discharges the `cannot-exercise-change` contract cannot trip the veto that guards it, so the agent refused to manufacture a BLOCK from compliant behavior; D8 alone carries the BLOCK coverage (new 8th veto code `verdict-not-posted` guards D7's criterion). Other flagged calls: anti-pattern vetoes (`gate-waived`, `unauthorized-discount`) kept verbatim per the D5 precedent though the sheet noted they're not-in-SKILL; `governance-reject-unscoped-release` keeps its id though the run now approves-with-conditions rather than rejects; a stale `growth-nurture-sequence` baseline entry was hand-pruned (`--update-baselines` appends, never prunes — worth knowing).
