...
- Agent Sub-Teams filter: a team picker appears above the assignee select in the new-ticket form, and as a row of filter pills at the top of the inline assignee autocomplete dropdown. Selecting a team narrows the visible members to the predefined set for that team. Built-in teams: **All Teams** (no filter), **Software Engineering**, **Content Engine**, **Growth Marketing**, **UX & Product Design**, **Data & Intelligence**, **Governance & Ops**, and **Health & Performance** — exact per-team membership lives in `AgentTeamService.DefaultTeams` (single source of truth; several support agents like producer/evaluator/documentalist appear in multiple teams). The human owner member (`owner`) is always included regardless of the selected team. Backed by `GigaClaw.Core/Services/AgentTeamService.cs` and `GigaClaw.Core/Models/AgentTeam.cs`; team display names are localised through `LocalizationService`.
